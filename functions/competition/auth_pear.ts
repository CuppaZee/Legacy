...
state_data.redirect}'">Close</button>
          ...
          //     client_id: config.pear.client_id,
          //     client_secret: config.pear.client_secret,
          //     grant_type: "authorization_code",
          //     code,
          //     redirect_uri: config.pear.redirect_uri,
          //   }),
          // });
          ...
          // var doc = db.collection('auth_pear').doc(user_id.toString())
          ...
          // if(otherTeam) {
          //   if(otherTeam.team_id === "pear" && !otherTeam.fixed[user_id]) {
          //     db.collection('data').doc('user_list_pear').update({
          //       [`fixed.${user_id}`]: true
          //     })
          //     res.send(
          //       `<!DOCTYPE html>
          ...
          // var { list } = (await db.collection('data').doc('user_list_pear').get()).data();
          // var discordmessage = `[PEAR] ${platform} ${username} | User #${list.length + 1}`;
          // db.collection('data').doc('user_list_pear').update({
