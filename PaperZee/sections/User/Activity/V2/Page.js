...
function UserActivityPage({ toggleDrawer, filters }) {
  ...
  var data = {
    captures: dataraw?.captures.map(i=>({
      ...i,
      activity: "capture"
    })).filter(i => filter(i, "captures")),
    deploys: dataraw?.deploys.map(i=>({
      ...i,
      activity: "deploy"
    })).filter(i => filter(i, "deploys")),
    captures_on: dataraw?.captures_on.map(i=>({
      ...i,
      activity: "capon"
    })).filter(i => filter(i, "captures_on")),
  }
