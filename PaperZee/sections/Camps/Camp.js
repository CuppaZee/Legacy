    ...
    <Card noPad>
      <TouchableRipple onPress={() => {
        ...
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          <View style={{ alignSelf: "stretch", borderTopRightRadius: 8, borderBottomRightRadius: open ? 0 : 8, borderLeftWidth: dark ? 2 : 0, borderLeftColor: dark ? level_colors[Math.min(Math.ceil(i.p / 50), 5)].bg : undefined, backgroundColor: dark ? undefined : level_colors[Math.min(Math.ceil(i.p / 50), 5)].bg, width: 60, alignItems: "center", justifyContent: "center" }}>
            <Text allowFontScaling={false} style={{ color: level_colors[Math.min(Math.ceil(i.p / 50), 5)].fg, fontSize: 16, ...font("bold") }}>{i.p?.toLocaleString?.() || "0"}</Text>
          </View>
        </View>
      </TouchableRipple>
      <Portal>
