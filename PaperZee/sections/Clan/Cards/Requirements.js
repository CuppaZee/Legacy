      ...
      {data?.levels?.length>0&&<View style={{flexDirection:"row"}}>
        <ScrollView horizontal={true} style={{flex:1}} contentContainerStyle={{flexDirection:"row",minWidth:'100%',paddingLeft:55*s}}>
          ...
          <View style={{flexDirection:"column",flexGrow:1,alignItems:"stretch",backgroundColor:darkBG??level_colors.ind}}>
            <View style={{height:24*s,padding:4*s}}><Text allowFontScaling={false} style={{textAlign:"center",...font("bold"),fontSize:12*s,color:darkBG&&level_colors.ind}}>Individual</Text></View>
            <View style={{flexDirection:"row"}}>
              {(data?.order?.individual??[]).map(i=><View key={`Individual${i}`} style={{flexGrow:1}}>
                <View style={{height:(96-19)*s,borderBottomWidth:2*s,borderBottomColor:level_colors.border,padding:4*s,alignItems:"center"}}>
                  ...
                  <Text allowFontScaling={false} numberOfLines={1} style={{color:darkBG&&level_colors.ind,textAlign:"center",...font("bold"),fontSize:12*s}}>{data?.requirements?.[i]?.top}</Text>
                  <Text allowFontScaling={false} numberOfLines={1} style={{color:darkBG&&level_colors.ind,textAlign:"center",...font(),fontSize:12*s}}>{data?.requirements?.[i]?.bottom}</Text>
                </View>
                {data?.levels?.map(l=><View key={l.id} style={{marginHorizontal:-1,height:24*s,padding:4*s,alignItems:"center",backgroundColor:darkBG??level_colors[l.id]}}>
                  <Text allowFontScaling={false} style={{textAlign:"center",width:'100%',fontSize:12*s,color:darkBG&&level_colors[l.id],...font()}}>{l.individual?.[i]?.toLocaleString()}</Text>
                </View>)}
              </View>)}
            </View>
          </View>
          ...
          {reward&&<View style={{flexDirection:"column",flexGrow:1,alignItems:"stretch",backgroundColor:darkBG??level_colors.ind}}>
            <View style={{height:24*s,padding:4*s}}><Text allowFontScaling={false} style={{textAlign:"center",...font("bold"),fontSize:12*s,color:darkBG&&level_colors.ind}}>Rewards</Text></View>
            <View style={{flexDirection:"row",marginRight:1}}>
              {(data?.order?.rewards??[]).map(i=><View key={`Reward${i}`} style={{flexGrow:1}}>
                <View style={{height:60,borderBottomWidth:2,borderBottomColor:level_colors.border,padding:4,alignItems:"center"}}>
                  ...
                  <Text allowFontScaling={false} numberOfLines={1} style={{textAlign:"center",...font("bold"),fontSize:10*s,color:darkBG&&level_colors.ind}}>{data?.rewards?.[i]?.short_name??data?.rewards?.[i]?.name.replace(/Virtual /,'V').replace(/Physical /,'P').replace(/Flat /,'').replace(/ Mystery/,'').replace(/THE /,'').replace(/ Wheel/,'W').replace(/Hammock/,'Hamm')}</Text>
                </View>
                {data?.levels?.map(l=><View key={l.id} style={{marginHorizontal:-1*s,height:24*s,padding:4*s,alignItems:"center",backgroundColor:darkBG??level_colors[l.id]}}>
                  <Text allowFontScaling={false} style={{textAlign:"center",width:'100%',fontSize:12*s,color:darkBG&&level_colors[l.id],...font()}}>{l.rewards?.[i]?.toLocaleString()}</Text>
                </View>)}
              </View>)}
            </View>
          </View>}
        ...
        <View style={{width:56*s,position:"absolute",left:0,top:0,borderRightWidth:2*s,borderRightColor:level_colors.border}}>
          <View style={{height:24*s,backgroundColor:darkBG??level_colors.null,padding:4*s}}><Text allowFontScaling={false} style={{...font("bold"),fontSize:12*s,color:darkBG&&level_colors.null}}>{data?.battle?.title?.slice(10)?.split(' ')?.[0]?.slice(0,3)} {data?.battle?.title?.slice(10)?.split(' ')?.[1]?.slice(2)}</Text></View>
          <View style={{height:(reward?60:77)*s,backgroundColor:darkBG??level_colors.null,flexDirection:"row",alignItems:"center",padding:4*s,borderBottomWidth:2*s,borderBottomColor:level_colors.border}}>
            <Text allowFontScaling={false} numberOfLines={1} ellipsizeMode="head" style={{fontSize:12*s,color:darkBG&&level_colors.null,...font()}}>Levels</Text>
          </View>
          {(data?.levels??[]).map(i=><View style={{backgroundColor:darkBG??level_colors[i.id],padding:4*s,height:24*s}} key={i.name}>
            <Text allowFontScaling={false} style={{fontSize:12*s,color:darkBG&&level_colors[i.id],...font()}}>{i.name}</Text>
          </View>)}
        </View>
      </View>}
