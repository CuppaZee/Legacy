...
var _clan_shadow = require('./Modules/clan/shadow');
exports.clan_shadow_v1 = _clan_shadow[0];
...
exports["bingo"] = functions.https.onRequest(async (req, res) => {
    return cors(req, res, async () => {
        var aaa = `<input id="x" value="" oninput="aaaa()" placeholder="username" type="text" /><a id="xd"></a><script>function aaaa(){
            ...
            xd.href = \`https://server.cuppazee.app/bingo?username=\${x.value}\`;
        ...
        }).join('')}</div><div>Generated by<br>https://server.cuppazee.app/bingo?username=${req.query.username}</div>`)
    })
})